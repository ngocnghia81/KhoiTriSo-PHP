...
export default function ApprovalsBooksPage() {
  const router = useRouter();
  const [books, setBooks] = useState<AdminBook[]>([]);
  const [loading, setLoading] = useState(true);
  const [rejectModal, setRejectModal] = useState<{ isOpen: boolean; bookId: number; title: string }>({
    isOpen: false,
    bookId: 0,
    title: ''
  });
  ...
  const loadBooks = async () => {
    try {
      setLoading(true);
      const response = await getPendingBooks({ page: pagination.page, pageSize: pagination.limit });
      setBooks(response.books);
      setPagination(response.pagination);
    } catch (error) {
      console.error('Failed to load books:', error);
      alert('Không thể tải danh sách sách chờ phê duyệt.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApproveBook = async (bookId: number) => {
    if (!confirm('Bạn có chắc muốn phê duyệt và xuất bản sách này?')) return;
    try {
      await approveBook(bookId);
      alert('Đã phê duyệt và xuất bản sách thành công!');
      loadBooks();
    } catch (error) {
      console.error('Failed to approve book:', error);
      alert('Không thể phê duyệt sách.');
    }
  };

  const handleRejectBook = async (reason: string) => {
    try {
      await rejectBook(rejectModal.bookId, reason);
      alert('Đã từ chối sách!');
      setRejectModal({ isOpen: false, bookId: 0, title: '' });
      loadBooks();
    } catch (error) {
      console.error('Failed to reject book:', error);
      alert('Không thể từ chối sách.');
    }
  };
    ...
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div className="sm:flex-auto">
          <h1 className="text-2xl font-semibold text-gray-900">Phê duyệt sách</h1>
          <p className="mt-2 text-sm text-gray-700">
            Xem xét và phê duyệt các sách đang chờ duyệt
          </p>
        </div>
        <button
          onClick={loadBooks}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white shadow-sm rounded-lg border border-gray-200">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Không có sách chờ phê duyệt</h3>
          <p className="mt-1 text-sm text-gray-500">Tất cả sách đã được xử lý.</p>
        </div>
      ) : (
            ...
            {books.map((book) => (
              <div key={book.id} className="p-6">
                <div className="flex items-start space-x-4">
                  <div className="flex-shrink-0">
                    {book.coverImage ? (
                      <img src={book.coverImage} alt={book.title} className="w-20 h-20 rounded-lg object-cover" />
                    ) : (
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex flex-col space-y-2 ml-4">
                        <div className="flex space-x-2">
                          ...
                          <button 
                            onClick={() => setRejectModal({ isOpen: true, bookId: book.id, title: book.title })} 
      ...
      <RejectModal 
        ...
        onClose={() => setRejectModal({ isOpen: false, bookId: 0, title: '' })} 
