    ...
    <div className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200 mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Phân bổ khóa học theo danh mục</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.topCourses && stats.topCourses.length > 0 ? (
              stats.topCourses.map((course: any, index: number) => (
                <div key={course.id} className="text-center p-4 bg-gray-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600 mb-1">{course.students}</div>
                  <div className="text-sm text-gray-600 mb-1">Học viên</div>
                  <div className="text-xs text-gray-500 truncate">{course.title}</div>
                </div>
              ))
            ) : (
              <p className="text-gray-500 col-span-full text-center py-8">Chưa có dữ liệu</p>
            )}
          </div>
        </div>
      </div>
